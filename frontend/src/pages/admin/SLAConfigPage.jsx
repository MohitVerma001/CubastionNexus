...
function SLARow({ config, onSave }) {
  ...
  const handleSave = () => {
    onSave({ ...config, first_response_hours: Number(firstResponse), resolution_hours: Number(resolution) });
    setEditing(false);
  };
          ...
          <div>
            {editing ? (
              <div className="flex gap-2">
                ...
                <Button icon={Save} size="sm" onClick={handleSave}>Save</Button>
              </div>
            ) : (
...
export default function SLAConfigPage() {
  const [configs, setConfigs] = useState(MOCK_SLA_CONFIGS);

  const handleSave = (updated) => {
    setConfigs(prev => prev.map(c => c.id === updated.id ? { ...updated, updated_at: new Date().toISOString() } : c));
  };
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="space-y-4">
        {configs.map(config => (
          <SLARow key={config.id} config={config} onSave={handleSave} />
        ))}
      </div>
